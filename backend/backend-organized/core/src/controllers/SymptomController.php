...
class SymptomController
{
	...
	public function create(): void
	{
		...
		// Validate required fields (use isset/array_key_exists to allow 0 values)
		foreach(['patient_id','date','pain_level','stiffness_level','fatigue_level'] as $k) {
			...
			// Check for empty strings or null (but allow 0)
		...
		try {
			...
			try {
				...
				// Create detailed notification for each doctor
				foreach ($doctorIds as $did) {
					...
					$notif->create((int)$did, 'PATIENT_SYMPTOM', $title, $message);
				}
				
				// Log for debugging
				error_log("Created symptom notifications for patient {$body['patient_id']} to " . count($doctorIds) . " doctors");
				
				// Additional debugging
				error_log("Patient ID: {$body['patient_id']}, Patient Name: $patientName");
				error_log("Doctor IDs: " . implode(', ', $doctorIds));
