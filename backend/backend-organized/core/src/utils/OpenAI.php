...
class OpenAI
{
    ...
    public function __construct()
    {
        ...
        $this->model = Env::get('OPENROUTER_MODEL', 'google/gemma-4-26b-a4b-it:free');
        $this->baseUrl = Env::get('OPENROUTER_BASE_URL', 'https://openrouter.ai/api/v1/chat/completions');
    }
    ...
    private function makeRequest(array $data): array
    {
        ...
        curl_setopt_array($ch, [
            ...
            CURLOPT_HTTPHEADER => [
                ...
                'X-Title: MyraJourney RA Assistant'
            ],
            CURLOPT_TIMEOUT => 30,
            CURLOPT_SSL_VERIFYPEER => false // Fix for local XAMPP/WAMP SSL certificate issues
        ]);
        ...
        if ($httpCode !== 200) {
            ...
            throw new \Exception('OpenRouter API error: HTTP ' . $httpCode . ' - ' . substr($response, 0, 200));
        }
