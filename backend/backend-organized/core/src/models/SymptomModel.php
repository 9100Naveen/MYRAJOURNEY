...
class SymptomModel
{
	...
	public function list(int $patientId, ?string $from, ?string $to): array
	{
		...
		$stmt=$this->db->prepare("SELECT * FROM symptom_logs $where ORDER BY `created_at` DESC, `date` DESC");
		$stmt->execute($p);
	...
	public function create(array $d): int
	{
		...
		$stmt=$this->db->prepare('INSERT INTO symptom_logs (patient_id, `date`, pain_level, stiffness_level, fatigue_level, joint_count, notes, created_at) VALUES (:pid,:date,:pain,:stiff,:fatigue,:joints,:notes,NOW())');
		$stmt->execute([
