...
class MedicationModel
{
    ...
    public function patientMedications(int $patientId, ?int $active, int $page, int $limit): array
    {
        ...
        $stmt = $this->db->prepare("
            SELECT pm.*, 
                   ...
                   d.signature_url as doctor_signature
    ...
    public function assign(array $d): int
    {
        ...
        $logFile = 'c:/Users/Admin/AndroidStudioProjects/myrajourney/myrajourney/backend/public/api_log.txt';
    ...
    {
        // First get the patient_id from the patient_medication
        $stmt = $this->db->prepare("SELECT patient_id FROM patient_medications WHERE id = :pmid");
        $stmt->execute([':pmid' => (int)$d['patient_medication_id']]);
        ...
        $stmt = $this->db->prepare("
            ...
            (patient_medication_id, patient_id, taken_at, status, notes, created_at)
            VALUES (:pmid, :pid, :taken_at, :status, :notes, NOW())
        ");
    ...
    public function getMedicationLogs(int $patientId): array
    {
        $stmt = $this->db->prepare("
            ...
            LEFT JOIN patient_medications pm ON ml.patient_medication_id = pm.id
            LEFT JOIN medications m ON pm.medication_id = m.id
