...
class ExerciseModel
{
    ...
    /**
     * Create a new RA exercise
     */
    public function createExercise($data)
    {
        $stmt = $this->db->prepare("
            INSERT INTO ra_exercises 
            (name, description, category, target_joints, difficulty_level, 
             video_url, animation_url, instructions, ra_benefits, created_at)
            VALUES 
            (:name, :description, :category, :target_joints, :difficulty_level,
             :video_url, :animation_url, :instructions, :ra_benefits, NOW())
        ");
